...
// Size of Page metadata in bytes
const PAGE_METADATA_SIZE: usize = 6;
// Size of Slot metadata in bytes
const SLOT_METADATA_SIZE: usize = 6;
// Status constants
const SLOT_USED: u16 = std::u16::MAX;
const SLOT_UNUSED: u16 = 0u16;
...
pub trait HeapPage {
    ...
    fn set_num_slots(&mut self, num_slots: SlotId);
    fn get_free_offset(&self) -> Offset;
    fn set_free_offset(&mut self, free_offset: Offset);
    fn get_slot_header_offset(&self, slot_id: SlotId) -> usize;
    fn get_slot_value_offset(&self, slot_id: SlotId) -> Offset;
    fn set_slot_value_offset(&mut self, slot_id: SlotId, value_offset: Offset);
    fn get_slot_value_size(&self, slot_id: SlotId) -> Offset;
    fn set_slot_value_size(&mut self, slot_id: SlotId, value_size: Offset);
    fn get_slot_status(&self, slot_id: SlotId) -> Offset;
    fn set_slot_status(&mut self, slot_id: SlotId, status: Offset);

    fn reclaim_space(&mut self, start_slot_id: SlotId, deleted_size: Offset);
}
...
impl HeapPage for Page {
    ...
    fn add_value(&mut self, bytes: &[u8]) -> Option<SlotId> {
        let value_size = bytes.len() as Offset;

        let num_slots = self.get_num_slots();

        let free_offset = self.get_free_offset();

        // // check if there is enough space for value and slot metadata
        let required_space = value_size + SLOT_METADATA_SIZE as Offset;
        if self.get_free_space() < required_space as usize {
            return None;
        }

        // copy bytes at calculated offset
        let new_value_offset = free_offset - value_size;
        self.data[new_value_offset as usize..free_offset as usize].copy_from_slice(bytes);

        // update metadata
        self.set_free_offset(new_value_offset);
        self.set_num_slots(num_slots + 1);

        self.set_slot_value_offset(num_slots, free_offset);
        self.set_slot_value_size(num_slots, value_size);
        self.set_slot_status(num_slots, SLOT_USED);
    ...
    fn get_value(&self, slot_id: SlotId) -> Option<Vec<u8>> {
        if slot_id >= self.get_num_slots() {
            return None; // invalid slot_id
        }

        // retrieve slot metadata
        let value_offset = self.get_slot_value_offset(slot_id);
        let value_size = self.get_slot_value_size(slot_id);

        if self.get_slot_status(slot_id) == SLOT_UNUSED || value_size == 0 {
            return None; // slot is unused or nothing to retrieve
        }

        // extract value
        let value = self.data[(value_offset as usize - value_size as usize)..value_offset as usize]
            .to_vec();
    ...
    fn delete_value(&mut self, slot_id: SlotId) -> Option<()> {
        ...
        if self.get_slot_status(slot_id) == SLOT_UNUSED || value_size == 0 {
            return None; // slot already deleted or nothing to delete
        }

        // set appropriate metadata
        self.set_slot_status(slot_id, SLOT_UNUSED);

        // move values to reclaim space
        self.reclaim_space(slot_id, value_size);

        Some(())
    }

    fn reclaim_space(&mut self, starting_slot_id: SlotId, deleted_size: Offset) {
        // loop through all slots after the deleted slot
        for slot_id in starting_slot_id + 1..self.get_num_slots() {
            // get metadata of the deleted slot
            let current_offset = self.get_slot_value_offset(slot_id);
            let current_size = self.get_slot_value_size(slot_id);
            let current_status = self.get_slot_status(slot_id);
            if current_status == SLOT_UNUSED {
                continue; // skip current slot if it is unused
            };

            // Calculate the new offset
            let new_offset = current_offset as usize - deleted_size as usize;

            // Move the data from the old location to the new location
            self.data[new_offset..new_offset + current_size as usize].copy_from_slice(
                &self.data
                    [current_offset as usize..current_offset as usize + current_size as usize],
            );

            // Update the slot metadata with the new offset
            self.set_slot_metadata(i, new_offset as Offset, current_size);
        }
    }
    ...
    #[allow(dead_code)]
    fn get_header_size(&self) -> usize {
        // Slot metadata size is fixed length, 6 bytes
        let slot_metadata_size = (self.get_num_slots() as usize) * SLOT_METADATA_SIZE;

        PAGE_METADATA_SIZE + slot_metadata_size
    }
    ...
    #[allow(dead_code)]
    fn get_free_space(&self) -> usize {
        // [HEADER -- FREE SPACE    free_offset->VALUES...]
        // Total used space = header size + free_offset
        let used_space = self.get_header_size() + (PAGE_SIZE - self.get_free_offset() as usize);

        PAGE_SIZE - used_space
    }

    // Header Metadata
    // First 2 bytes for slot_id
    // Second 2 bytes for number of slots currently allocated
    // Last 2 bytes for the first free offset available
    ...
    // Slot Metadata
    // First 2 bytes are for value offset
    // Second 2 bytes are for value length
    // Last 2 bytes are for whether the slot is free

    fn get_slot_header_offset(&self, slot_id: SlotId) -> usize {
        PAGE_METADATA_SIZE + (slot_id as usize) * SLOT_METADATA_SIZE
    }

    fn get_slot_value_offset(&self, slot_id: SlotId) -> Offset {
        let slot_offset = self.get_slot_header_offset(slot_id);
        Offset::from_le_bytes(self.data[slot_offset..slot_offset + 2].try_into().unwrap())
    }

    fn set_slot_value_offset(&mut self, slot_id: SlotId, value_offset: Offset) {
        let slot_offset = self.get_slot_header_offset(slot_id);
        self.data[slot_offset..slot_offset + 2].copy_from_slice(&value_offset.to_le_bytes());
    }

    fn get_slot_value_size(&self, slot_id: SlotId) -> u16 {
        let slot_offset = self.get_slot_header_offset(slot_id) + 2;
        Offset::from_le_bytes(self.data[slot_offset..slot_offset + 2].try_into().unwrap())
    }

    fn set_slot_value_size(&mut self, slot_id: SlotId, value_size: u16) {
        let slot_offset = self.get_slot_header_offset(slot_id) + 2;
        self.data[slot_offset..slot_offset + 2].copy_from_slice(&value_size.to_le_bytes());
    }

    fn get_slot_status(&self, slot_id: SlotId) -> u16 {
        let slot_offset = self.get_slot_header_offset(slot_id) + 4;
        u16::from_le_bytes(self.data[slot_offset..slot_offset + 2].try_into().unwrap())
    }

    fn set_slot_status(&mut self, slot_id: SlotId, status: u16) {
        let slot_offset = self.get_slot_header_offset(slot_id) + 4;
        self.data[slot_offset..slot_offset + 2].copy_from_slice(&status.to_le_bytes());
    }
}
